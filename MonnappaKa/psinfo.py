...
# Twitter: @monnappa22
# Description: Volatility Plugin to display process related information and suspicous memory regions 
...
class PsInfo(vadinfo.VADDump):
    """Displays process related information and suspicious memory regions"""
    ...
    def update_proc_peb_info(self, psdata):
        ...
        for proc in psdata:
            ...
            if proc.Peb: 
                ...
                self.proc_peb_info[pid].extend(["No PEB",
                                                Address(0),
                                                Address(0),
                                                Hex(0),
                                                "No PEB",
    ...
    def get_similar_procs(self, procid):
        ...
        full_path = proc_peb_info[procid][10]
        self.similar_procs.append([pname, procid, ppname, ppid, create_time, full_path])
        ...
        for pid in proc_peb_info:
            ...
            if pname == proc_peb_info[pid][2]:
                ...
                full_path = proc_peb_info[pid][10]
                parent_name, parent_id = parent_proc_info[pid]
                self.similar_procs.append([proc_name, pid, parent_name, parent_id, creation_time, full_path])
    ...
    def render_text(self, outfd, data):
        for (proc_peb_info, proc_vad_info, parent_proc_info, similar_procs) in data:
            ...
            (vad_filename, vad_baseaddr, vad_size, vad_protection, vad_tag) = proc_vad_info
            ...
            for similar_proc in similar_procs:
                (process_name, process_id, parent_name, parent_id, creation_time, full_path) = similar_proc
                outfd.write("{0}\n".format(full_path))
                ...
                if obj.Object("_IMAGE_DOS_HEADER", offset = vad.Start, vm = addr_space).e_magic != 0x5A4D:
                    ...
                    if (vad_prot == "PAGE_EXECUTE_READWRITE"):
                        ...
                        if self._config.DUMP_DIR:
                            filename = os.path.join(self._config.DUMP_DIR,"process.{0}.{1:#x}.dmp".format(pid, sus_addr))
                    ...
                    if vad_prot == "PAGE_EXECUTE_READWRITE":
                        ...
                        outfd.write("\t{0:#x}({1})  Protection: {2}  Tag: {3}\n".format(sus_addr,
                                                                                        ...
                                                                                        str(vad.Tag or "")))
                        if self._config.DUMP_DIR:
                            filename = os.path.join(self._config.DUMP_DIR,"process.{0}.{1:#x}.dmp".format(pid, 
                                                                                                          sus_addr))
                            self.dump_vad(filename, vad, addr_space)
